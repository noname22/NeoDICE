 ...
 */
local int  mem_read     OF((char *buf, unsigned size));
local void flush_outbuf OF((unsigned w, unsigned size));
 ...
 */
local void flush_outbuf(w, size)
    unsigned w;    /* value to flush */
    unsigned size; /* it size in bytes (0, 1 or 2) */
{
    if (zfile == NULL) {
    ...
    int header;    /* true if block header must be written */
{
    ...
    if (zfile) {
        ...
        if (ferror(zfile)) error ("write error on zip file");
    } else if (out_offset + (ulg)len > out_size) {
        error("output buffer too small for in-memory compression");
    } else {
 ...
 */
local int mem_read(buf, size)
     char *buf;
     unsigned size; 
{
    if (in_offset < in_size) {
        ulg block_size = in_size - in_offset;
        if (block_size > (ulg)size) block_size = (ulg)size;
